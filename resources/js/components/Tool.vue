    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow pt-2">
      <div class="flex border-40 remove-bottom-border px-8">
        <div class="w-full mt-6 pb-2">
          ...
          <trix-editor
            ref="editor"
            class="trix-content prose !max-w-full prose-sm dark:prose-invert w-full form-control form-input form-input-bordered py-3 h-auto mt-2"
            input="commenter"
            dusk="commenter-textarea"
            @trix-change="comment = $event.target.value"
      ...
      <div class="flex justify-between px-8 pb-4 border-b border-40">
        ...
        <button
          class="flex-shrink-0 shadow rounded focus:outline-none focus:ring bg-primary-500 hover:bg-primary-400 active:bg-primary-600 text-white dark:text-gray-800 inline-flex items-center font-bold px-4 h-9 text-sm mb-3 mt-4"
          type="submit"
          dusk="commenter-save"
          @click="createComment"
        >
...
export default {
  ...
  computed: {
    ...
    hasComments() {
      return Boolean(this.data.resources && this.data.resources.length)
    },
